...
public class InitQuark implements IIntegrationModule {
    ...
    private void initHeatables() {
        ...
        if(ModIds.immersive_engineering.isLoaded) {
            if (!ConfigHandlerMI.heatables.immersive_engineering.alloy_smelter.foxhound) {
                HeatableUtils.blacklistHeatable(EntityFoxhound.class, TileEntityAlloySmelter.class);
            }
            if (!ConfigHandlerMI.heatables.immersive_engineering.blast_furnace.foxhound) {
                HeatableUtils.blacklistHeatable(EntityFoxhound.class, TileEntityBlastFurnace.class);
            }
            if (!ConfigHandlerMI.heatables.immersive_engineering.coke_oven.foxhound) {
        ...
        if(ModIds.industrial_craft.isLoaded) {
            if (!ConfigHandlerMI.heatables.industrial_craft.blast_furnace.foxhound) {
                HeatableUtils.blacklistHeatable(EntityFoxhound.class, ic2.core.block.machine.tileentity.TileEntityBlastFurnace.class);
            }
            if (!ConfigHandlerMI.heatables.industrial_craft.fermenter.foxhound) {
